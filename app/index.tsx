...
import { Image, Pressable, ScrollView, StyleSheet, Text, TouchableOpacity, View, Dimensions, Alert } from "react-native";
  ...
  const loadEntries = useCallback(() => {
    try {
      ...
      const parsed = items.filter(i => !(i instanceof Directory))
        ...
        .sort((a:any, b:any) => {
        // This assumes entry.json.date is a valid date string (e.g., "2023-10-27")
        // If it's a custom format, you may need to parse it specifically
        const dateA =a.json.id;
        const dateB = b.json.id;
...
const exportAllToPDF = async () => {
  try {
    ...
    // 1. Loop through entries and capture
    for (const entry of entries) {
      setSharingEntry(entry);
      // Wait for the UI to update the template
      await new Promise(resolve => setTimeout(resolve, 600));  
      const uri = await viewShotRef.current?.capture?.();
      
      if (uri) {
        // 2. Convert the captured file to a Base64 string
      ...
      <ScrollView style={{ flex: 1 }}>
        {entries.map((entry, index) => (
          <View key={index} style={styles.card}>
            <Pressable onPress={() => router.push(`/editEntry?path=${encodeURIComponent(entry.file.uri)}`)}>
              {entry.json.image && <Image source={{ uri: entry.json.image }} style={{ width: "100%", height: 200 }} />}
              <Text style={styles.text} numberOfLines={3}>{entry.json.text}</Text>
            </Pressable>
            <View style={styles.cardFooter}>
              <Text style={styles.date}>{entry.json.date}</Text>
              <View style={styles.rightActions}>
                <Pressable style={styles.delete} onPress={() => handleExport(entry, 'save')}>
                  <FontAwesome5 name="save" size={20} color="#555" />
                </Pressable>
                <Pressable style={styles.delete} onPress={() => handleExport(entry, 'share')}>
...
const styles = StyleSheet.create({
  ...
  delete: { padding: 15 },
  floatingBtn: { position: "absolute", bottom: 80, right: 30, backgroundColor: "#fff", padding: 10, borderRadius: 50, elevation: 10 }
});